...
import { apiExportThemeDesignPdf, apiGetThemeDesignById, apiUpdateThemeDesign } from '@/services/SaveDesignService'
                ...
                <div className="bg-white/80 backdrop-blur-md border border-[#E8E0D9]/60 rounded-3xl p-6 md:p-10 shadow-xl shadow-[#8B5A3C]/5">
                    ...
                    {loading ? (
                        <div className="flex flex-col justify-center items-center py-24 gap-4">
                            ...
                            <p className="text-[#8B5A3C] text-sm animate-pulse">Loading simulation specifications...</p>
                        </div>
                    ) : !themeDesignData ? (
                        <div className="py-24 text-center">
                            <p className="text-gray-400 font-medium">No theme design details could be found.</p>
                            <button 
                        ...
                        <>
                            ...
                            <div className="lg:col-span-5 flex flex-col items-center justify-center w-full">
                                <div className="relative group flex justify-center items-center h-[380px] md:h-[480px] w-full bg-gradient-to-tr from-[#FAF8F5] to-white border border-[#E8E0D9]/80 rounded-2xl overflow-hidden p-6 shadow-inner">
                                    <Image
                                        src={themeDesignData?.ThemeImage || '/img/table-form/full-venue.png'}
                            ...
                            <div className="lg:col-span-7 flex flex-col h-full justify-between">
                                <div>
                                    <div className="mb-6">
                                        ...
                                        <p className="text-gray-600 leading-relaxed text-sm text-justify">
                                            {themeDesignData?.description || "Experience table coordination redefined. Explore fully interactive layouts tailored seamlessly to your venue aesthetics."}
                                        </p>
                                    </div>
                                    ...
                                    <div className="bg-[#FAF8F5] border border-[#E8E0D9]/50 rounded-2xl p-5 mb-6">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            <SpecCard title="Table Shape" value={themeDesignData?.config_json?.table_shape || "Circle"} />
                                            <SpecCard title="Table Scale" value={`${themeDesignData?.config_json?.table_scale || "300"} cm`} />
                                            <SpecCard title="Seating Capacity" value={`${themeDesignData?.config_json?.table_sitting || "6"} Seats`} />
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white border border-[#E8E0D9]/70 rounded-2xl shadow-sm overflow-hidden mb-8">
                                        ...
                                        <div className="divide-y divide-gray-100 max-h-[280px] overflow-y-auto">
                                            {Object.entries(themeDesignData?.design_specifications || {}).map(([category, options]) => (
                                                <div key={category} className="px-5 py-3 hover:bg-[#FAF8F5]/40 transition flex justify-between items-start gap-4">
                                                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wide mt-1">
                                                        {category}
                                                    </span>
                                                    <div className="text-right">
                                                        {typeof options === 'object' ? (
                                                            Object.entries(options).map(([optName, optVal]) => (
                                                                <span key={optName} className="inline-block bg-white border border-[#E8E0D9] text-[#2C1810] text-[11px] font-semibold px-2.5 py-1 rounded-md ml-1.5 shadow-sm">
                                                                    {optName}: {optVal}
                                                                </span>
                                                            ))
                                                        ) : (
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 w-full mt-auto">
                                    <button
                                        ...
                                        disabled={isSaving}
                                    >
                                        {isSaving ? <Spinner size={16} customColorClass="text-[#A0522D]" /> : <FiSave size={16} />}
                                        <span>{isSaving ? 'Saving...' : 'Save Theme Design'}</span>
                                    </button>
                        ...
                        {themeDesignData?.products && themeDesignData.products.length > 0 && (
                            <div className="mt-12 border-t border-[#E8E0D9]/60 pt-10">
                                <h3 className="text-xl font-bold text-[#2C1810] mb-6 flex items-center gap-2">
                                    <FiLayers className="text-[#A0522D]" /> Included Products in Theme Setup
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                    {themeDesignData.products.map((prod) => (
                                        <div key={prod.id} className="bg-white border border-[#E8E0D9]/50 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col justify-between group">
                                            <div>
                                                ...
                                                <div className="p-4">
                                                    <h4 className="font-bold text-sm text-[#2C1810] line-clamp-1 mb-1" title={prod.title}>
                                                        {prod.title}
                                                    </h4>
                                                    <p className="text-xs text-gray-500 line-clamp-2 mb-3 leading-relaxed">
                                                        {prod.description || "Beautifully coordinated theme accent piece."}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="p-4 border-t border-gray-50 flex items-center justify-between">
                                                <span className="text-xs text-gray-400 font-medium">Rental Price</span>
