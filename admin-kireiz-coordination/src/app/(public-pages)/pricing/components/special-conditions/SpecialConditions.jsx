  ...
  useEffect(() => {
    ...
    if (mode === "edit" && initialData) {
      reset({
        ...
        discountPercentage: initialData.discount_percentage?.toString() || "",
      });
  ...
  const handleSave = async (values) => {
    ...
    try {
      const payload = {
        ...
        condition_type: values.conditionType?.value || "",
  ...
  const CardSkeleton = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {Array.from({ length: 3 }).map((_, i) => (
        <div
          key={i}
          className="bg-white border border-[#E2E8F0] rounded-2xl p-6 animate-pulse"
        >
