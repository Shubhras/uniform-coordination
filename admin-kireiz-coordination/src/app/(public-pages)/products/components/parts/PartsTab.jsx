...
import { FiSearch, FiEdit2, FiCopy, FiPlus, FiTrash2, FiChevronLeft, FiChevronRight } from "react-icons/fi";
...
const PartsTab = () => {
    ...
    const filteredParts = parts.filter((p) => {
        ...
        const matchesCategory =
            category.value === "all" ||
            p.category?.toLowerCase() === category.value;
    ...
    const GridSkeleton = () => (
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="border border-[#1C2C5633] rounded-xl animate-pulse">
