...
### NPM Scripts
```bash
npm run dev          # Start dev server on port 7003 (accessible on 0.0.0.0)
npm run build        # Production build
npm run gcp-build    # GCP build with 8GB memory limit
npm run start        # Start production server on port 7003
npm run lint         # ESLint
...
```js
{
  ...
  watch: true,
  env: { NODE_ENV: "production", PORT: 7003 }
}
```
