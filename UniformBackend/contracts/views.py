...
class QuotationSendAPIView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(description="Final agreement sent successfully"),
    ...
    def post(self, request):
        try:
            ...
            if not env:
                return Response(
                    {"message": "No DocuSign envelope found"},
                    status=400
                )

            if env.agreement_status != "client_signed":
                return Response(
                    {"message": "Client has not signed yet"},
                    status=400
                )

            # Prevent duplicate final send
            if env.agreement_status == "final_sent":
                return Response(
                    {"message": "Final agreement already sent"},
                    status=400
                )

            # Send final signed PDF (DocuSign logic untouched)
            ...
            return Response(
                {
                    "status": False,
                    "message": "Failed to send final agreement",
                    "error": str(e),
                },
                status=500
            )
