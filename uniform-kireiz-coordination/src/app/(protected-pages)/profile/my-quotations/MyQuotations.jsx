...
const getNormalizedStatus = (quotation) => {
  ...
  if (rawStatus.includes("declin") || rawStatus.includes("reject"))
    return "declined";
  if (rawStatus.includes("pend")) return "pending";
  if (rawStatus.includes("submit")) return "submitted";
  if (rawStatus.includes("receiv") || rawStatus.includes("review"))
    return "received";
...
const MyQuotations = () => {
  ...
  const filteredQuotations = statusFilter
    ...
    : quotations;
  const visibleTotalCount = statusFilter
  ...
  return (
    <>
      <div className="w-full rounded-2xl bg-white px-4 py-6 shadow-sm md:px-6">
        <div className="mb-6">
          <h2 className="text-[30px] font-semibold text-[#003560]">
            My Quotations
          </h2>
        </div>
          ...
          <div className="flex gap-3 lg:w-auto">
            <Select
              ...
              styles={{
                control: (base) => {
                  const activeColor =
                    statusStyles[statusFilter]?.text || "#003560";
                  return {
                    ...base,
                    borderRadius: "10px",
                    ...
                    backgroundColor: statusFilter
                      ? statusStyles[statusFilter]?.bg || "white"
                      : "white",
                    display: "flex",
                    alignItems: "center",
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead className="bg-[#F2F5FB]">
                  <tr>
                    <th className="px-4 py-4 text-left text-sm font-medium text-[#4B5563]">
                      Request ID
                    </th>
                    <th className="px-4 py-4 text-left text-sm font-medium text-[#4B5563]">
                      Product Name
                ...
                <tbody>
                  {filteredQuotations.length ? (
                    filteredQuotations.map((item, index) => (
                      ...
                      >
                        <td className="px-4 py-4 text-sm font-medium text-[#003560]">
                          {item.requestId}
                        </td>
                        <td className="px-4 py-4 text-sm text-[#003560]">
                          {item.productName}
                        </td>
                        <td className="px-4 py-4 text-sm text-[#003560]">
                          {item.quantity}
                    ...
                    <tr>
                      ...
                      >
                        {errorMessage
                          ...
                          : "No quotations found for the selected status."}
                      </td>
                    </tr>
                  )}
                </tbody>
